...
const Login = () => {
  ...
  async function sendOtp() {
    try {
      setOtpBtnDisabled(true);
      await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/send-otp`,
        JSON.stringify({
          authMethod: authMethod,
          ...(authMethod === "email" && { email: formik.values.user_id }),
          ...(authMethod === "phone" && {
            phone_number: formik.values.user_id,
          }),
      ...
      await axios
        .post(
          ...
          JSON.stringify({
            ...
            organization_code:
              process.env.NEXT_PUBLIC_ORGANISATION.toUpperCase(),
          }),
          {
            withCredentials: true,
            headers: {
        ...
        .then(async (res) => {
          var hashedValue = bcrypt.hashSync(
          ...
          localStorage.setItem(
            "profilePic",
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/${res.data.profile_pic}`
          );
          ...
          if (res.data.profile_complete == 1)
      ...
      <Box minH={"100vh"}>
        ...
        >
                      ...
                      <VStack py={8} alignItems={"flex-start"}>
                        <Box mb={4}>
                          ...
                          <InputGroup w={["xs", "sm"]}>
                            <InputLeftAddon
                              children={
                                authMethod == "phone" ? <FiPhone /> : <FiMail />
                              }
                              rounded={"full"}
                            />
                            <Input
                        ...
                        <HStack spacing={6} pt={4}>
                          ...
                          <RadioGroup
                            ...
                            onChange={(value) => setLoginPreference(value)}
                          >
                            <HStack spacing={4}>
          ...
          <VStack
            ...
            justifyContent={"center"}
            h={"full"}
            borderRadius={["unset", "0 12 12 0"]}
          >
            <Image src={"../auth.png"} />
          </VStack>
        </HStack>
        <Text mt={8} fontSize={"xs"} w={"full"} textAlign={"center"}>
