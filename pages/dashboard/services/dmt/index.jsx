...
const Dmt = () => {
  ...
  useEffect(() => {
    ...
    if (dmtProvider == "paysprint") {
      BackendAxios.get(`/api/paysprint/dmt/banks/${serviceId}`)
        .then((res) => {
    ...
    if (dmtProvider == "eko") {
      BackendAxios.get(`/api/eko/aeps/fetch-bank/${serviceId}`)
        .then((res) => {
  ...
  const registrationFormik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (dmtProvider == "eko") {
        BackendAxios.post(
          `api/${dmtProvider}/dmt/create-customer/${serviceId}`,
          {
            ...values,
  ...
  const paymentFormik = useFormik({
    ...
    onSubmit: (values) => {
      setIsLoading(true);
      if (dmtProvider == "paysprint") {
        BackendAxios.post(`/api/paysprint/dmt/initiate-payment/${serviceId}`, {
          ...values,
          customerId: customerId,
        })
          .then((res) => {
    ...
    BackendAxios.post(`/api/paysprint/bank/bank-verify`, {
      ...
      .then((res) => {
        ...
        if (res.data.data.account_exists) {
          setIsLoading(false);
          addRecipientFormik.setFieldValue(
            "beneficiaryName",
          ...
          if (dmtProvider == "paysprint") {
            ...
            if (res.data.response_code == 1) {
              ...
              setCustomerRemainingLimit(
                parseInt(res.data.data.bank1_limit) +
                  parseInt(res.data.data.bank2_limit) +
                  parseInt(res.data.data.bank3_limit)
              );
  ...
  function fetchRecipients() {
    ...
    if (dmtProvider == "eko") {
      BackendAxios.get(
        `/api/eko/dmt/recipient-list/${serviceId}?customerId=${customerId}`
      )
        .then((res) => {
          if (res.data?.data?.recipient_list?.length) {
            setIsLoading(false);
            setRecipients(
              res.data.data.recipient_list.map((recipient) => {
                return {
  ...
  return (
    <>
      <DashboardWrapper titleText={"Domestic Money Transfer"}>
        ...
        <Box p={6} bg={"white"} boxShadow={"md"} rounded={12} mt={6}>
          <HStack>
            {showSenderIdInput ? (
              <Box>
          ...
          {customerStatus == "unregistered" ? (
            <>
              <Text pt={16} pb={4}>
                The customer isn't registered yet. Please fill the form before
                initaing the transaction.
              </Text>
              <Stack
              ...
              >
                <FormControl id="customerName" w={["full", "xs"]} isRequired>
                  ...
                  <Input
                    ...
                    value={registrationFormik.values.customerName}
                ...
                <FormControl id="customerDob" w={["full", "xs"]} isRequired>
                  ...
                  <Input
                    type={"date"}
                    name="customerDob"
                    value={registrationFormik.values.customerDob}
                    onChange={registrationFormik.handleChange}
                  />
                </FormControl>
              </Stack>
              ...
              <FormControl id="street" w={["full"]} isRequired>
                ...
                <Input
                  name="street"
                  placeholder="Enter Name"
                  value={registrationFormik.values.street}
                  onChange={registrationFormik.handleChange}
                />
              </FormControl>
              <Stack
              ...
              >
                ...
                <FormControl id="pincode" w={["full"]} isRequired>
                  ...
                  <Input
                    ...
                    maxLength={6}
                    name="pincode"
                    value={registrationFormik.values.pincode}
                    onChange={registrationFormik.handleChange}
                  />
                </FormControl>
                <FormControl id="state" w={["full"]} isRequired>
                  ...
                  <Select
                    ...
                    value={registrationFormik.values.state}
                    onChange={registrationFormik.handleChange}
                  >
          ...
          {customerStatus == "registered" ? (
            <>
              <Text pt={16} pb={4}>
                Following details were formed. You can initate the transaction
                now.
              </Text>
              <Stack
                direction={["column", "row"]}
                ...
                <VStack w={["full", "lg"]} alignItems={"flex-start"}>
                  <Box w={"inherit"} p={4} rounded={12} my={4} bg={"blue.100"}>
                    ...
                    <Stack direction={["column", "row"]} spacing={4} py={2}>
                      ...
                      <Text>
                        ₹ {customerTotalLimit.toLocaleString("en-IN")}
                      </Text>
                    </Stack>
                    <Stack direction={["column", "row"]} spacing={4} py={2}>
                      <Text fontWeight={"semibold"}>Remaining Limit: </Text>
                      <Text>
                        ₹ {customerRemainingLimit.toLocaleString("en-IN")}
                      </Text>
                    </Stack>
                    <Stack direction={["column", "row"]} spacing={4} py={2}>
                ...
                <Box w={["full", "md"]}>
                  <HStack w={"full"} justifyContent={"flex-end"}>
                    <Button
                      ...
                      onClick={() => setNewRecipientModal(true)}
                    >
                      Add New Account
                    </Button>
                  </HStack>
                          ...
                          return (
                            ...
                            >
                              ...
                              <Box w={"full"} p={4} fontSize={"xs"}>
                                ...
                                <Text mb={2}>
                                  <span style={{ paddingRight: "1.20rem" }}>
                                    <b>Bank IFSC: </b>
                                  </span>
                                  {item.bankIfsc}
                                </Text>
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  colorScheme={"red"}
                                  onClick={() =>
                                    deleteRecipient(item.beneficiaryId)
                                  }
                                >
                                  <Text>Delete</Text>
                                </Button>
      ...
      <Modal
        ...
        onClose={() => setPaymentConfirmationModal(false)}
      >
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody
            ...
            textAlign={"center"}
          >
            Are you sure you want to send ₹ <b>{paymentFormik.values.amount}</b>{" "}
            to <b>{paymentFormik.values.beneficiaryName}</b>
            <br />
            <br />
            ...
            >
              <PinInput
                otp
                onChange={(value) => paymentFormik.setFieldValue("mpin", value)}
              >
      ...
      <Modal
        isOpen={newRecipientModal}
        onClose={() => setNewRecipientModal(false)}
      >
        <ModalOverlay />
        <ModalContent>
          ...
          >
              ...
              >
                {bankList.map((bank, key) =>
                  dmtProvider == "paysprint" ? (
                    <option key={key} value={bank.bank_id}>
                      {bank.name}
                    </option>
                  ) : dmtProvider == "eko" ? (
                    <option key={key} value={bank.value}>
                      {bank.label}
                    </option>
                  ) : null
                )}
            ...
            <FormControl id="ifsc" pb={4}>
              ...
              <Input
                value={addRecipientFormik.values.ifsc}
                onChange={addRecipientFormik.handleChange}
              />
            </FormControl>
            <FormControl id="accountNumber" pb={4}>
              ...
              <Input
                type={"number"}
                value={addRecipientFormik.values.accountNumber}
                onChange={addRecipientFormik.handleChange}
              />
            </FormControl>
            <FormControl id="beneficiaryName" pb={4}>
              <FormLabel>Beneficiary Name</FormLabel>
              <Input
                value={addRecipientFormik.values.beneficiaryName}
                onChange={addRecipientFormik.handleChange}
              />
            </FormControl>
            <HStack justifyContent={"flex-end"} pt={2}>
              <Button size={"xs"} onClick={getAccountHolderName}>
                Get Name
              </Button>
            </HStack>
            <FormControl id="beneficiaryPhone" pb={4}>
              ...
              <Input
                value={addRecipientFormik.values.beneficiaryPhone}
                onChange={addRecipientFormik.handleChange}
              />
            </FormControl>
            ...
            <HStack gap={2} mt={8}>
              <FormControl id="address">
                ...
                <Input
                  value={addRecipientFormik.values.address}
                  onChange={addRecipientFormik.handleChange}
                />
              </FormControl>
              <FormControl id="pincode">
                <FormLabel>PINCODE</FormLabel>
                <Input
                  value={addRecipientFormik.values.pincode}
                  onChange={addRecipientFormik.handleChange}
                />
              </FormControl>
            </HStack>
          ...
          <ModalFooter>
            <Button variant="ghost" onClick={() => setNewRecipientModal(false)}>
              Cancel
            </Button>
            <Button
              colorScheme="blue"
              mr={3}
              onClick={addRecipientFormik.handleSubmit}
            >
              Add Account
            </Button>
          </ModalFooter>
