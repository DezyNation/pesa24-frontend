...
const UserLedger = () => {
  ...
  function fetchLedger(pageLink) {
    BackendAxios.get(
      pageLink ||
        `/api/parent/users-transactions${userId ? `/${userId}` : ""}?from=${
  ...
  const merchantCellRenderer = (params) => {
    return (
      <Text>
        ({params.data.trigered_by}) {params.data.name}
      </Text>
    );
  };
  ...
  useEffect(() => {
    if (Router.isReady && user_id) {
      BackendAxios.get(
        `/api/parent/users-transactions/${user_id}?from=${
          Formik.values.from + (Formik.values.from && "T00:00")
        }&to=${Formik.values.to + (Formik.values.to && "T23:59")}&search=${
          Formik.values.search
        }&page=1`
      ...
      <DashboardWrapper pageTitle={"User Ledger"}>
        <Box p={4}>
          <FormControl w={["full", "xs"]}>
            ...
            <InputGroup>
              ...
              <InputRightAddon
                ...
                onClick={() =>
                  fetchLedger(
                    `/api/parent/users-transactions/${userId}?from=${
                      Formik.values.from + (Formik.values.from && "T00:00")
                    }&to=${
                      Formik.values.to + (Formik.values.to && "T23:59")
                    }&search=${Formik.values.search}&page=1`
                  )
                }
              />
            </InputGroup>
          ...
          <HStack py={4} justifyContent={'flex-end'}>
            <Button colorScheme="twitter" onClick={()=>fetchLedger()}>Search</Button>
          </HStack>
