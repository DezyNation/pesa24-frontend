...
const Index = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {
      headerName: "Fastag ID",
      field: "metadata",
      cellRenderer: "fastagIdCellRenderer",
    },
    {
  ...
  function generateReport(doctype) {
    ...
    BackendAxios.get(
      ...
      .then((res) => {
        ...
        if (doctype == "excel") {
          fileDownload(res.data, "FastagReport.xlsx");
        } else {
          fileDownload(res.data, "FastagReport.pdf");
        }
      })
      .catch((err) => {
        setReportLoading(false);
  ...
  const fastagIdCellRenderer = (params) => {
    return <Text>{JSON.parse(params?.data?.metadata)?.transaction_id}</Text>;
  };

  return (
    <>
      <DashboardWrapper pageTitle={"Fastag Reports"}>
        <HStack>
          <Button
          ...
          >
            <AgGridReact
              ...
              components={{
                ...
                fastagIdCellRenderer: fastagIdCellRenderer,
              }}
