...
const Index = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    },
    {
      ...
      field: "metadata",
      cellRenderer: "applicationIdCellRenderer",
    },
    {
  ...
  function generateReport(doctype) {
    ...
    BackendAxios.get(
      ...
      .then((res) => {
        ...
        if (doctype == "excel") {
          fileDownload(res.data, "PanReport.xlsx");
        } else {
          fileDownload(res.data, "PanReport.pdf");
        }
      })
      .catch((err) => {
        setReportLoading(false);
  ...
  const applicationIdCellRenderer = (params) => {
    return <Text>{JSON.parse(params?.data?.metadata)?.application_id}</Text>;
  };

  return (
    <>
      <DashboardWrapper pageTitle={"PAN Application Reports"}>
        <HStack>
          <Button
        ...
        <Stack p={4} spacing={8} w={"full"} direction={["column", "row"]}>
          ...
          <FormControl w={["full", "xs"]}>
            <FormLabel>Ref ID or Application ID</FormLabel>
          ...
          >
            <AgGridReact
              ...
              components={{
                ...
                applicationIdCellRenderer: applicationIdCellRenderer,
              }}
